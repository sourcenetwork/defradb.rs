...
async fn rust_circuit_breaker_threshold_1_trips_immediately() {
    ...
    // Setup: create policy + protected doc while Vera is healthy
    ...
    // All subsequent requests are denied immediately without even attempting Vera.
    ...
    assert_eq!(
        ...
        0,
        "threshold=1: Bob denied after Vera down (fail-closed)"
    );
}

/// Short cache TTL (2s) causes cached policy entries to expire quickly.
/// After expiry, the node must re-query Vera for the policy.
///
...
async fn rust_cache_ttl_expiry_with_short_ttl() {
    ...
    // After TTL expiry, the node re-queries Vera for policy.
    // This should still work because Vera is healthy.
    ...
    assert_eq!(
        ...
        1,
        "Alice should still see 1 doc after cache expiry (re-fetched from Vera)"
    );
