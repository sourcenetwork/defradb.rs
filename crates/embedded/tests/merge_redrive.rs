...
use p2p::replication_policy::{OutboundBlock, OutboundPath, PolicyPeer, ReplicationPolicy};
...
/// Allows every block and records what was offered, so a push can be counted.
#[derive(Default)]
struct CountOutbound {
    offered: Mutex<Vec<(OutboundPath, String, Cid)>>,
}

impl CountOutbound {
    fn pushes_of(&self, doc_id: &str) -> Vec<Cid> {
        self.offered
            .lock()
            .unwrap()
            .iter()
            .filter(|(path, offered, _)| *path == OutboundPath::Push && offered == doc_id)
            .map(|(_, _, cid)| *cid)
            .collect()
    }
}
...
#[async_trait]
impl ReplicationPolicy for CountOutbound {
    async fn may_send(
        &self,
        _peer: &PolicyPeer<'_>,
        path: OutboundPath,
        ...
        for doc_id in block.doc_ids {
            offered.push((path, doc_id.clone(), *block.cid));
        }
        Ok(true)
    }
}
...
async fn a_composite_merged_by_redrive_reaches_this_nodes_replicators() -> Result<()> {
    let outbound = Arc::new(CountOutbound::default());
    let author = plain_node().await?;
    let middle = NodeBuilder::default()
        .with_iroh(iroh_config())
        .with_access_hooks(
            AccessHooks::new(["Note"])
                .with_merge_validator(Arc::new(AwaitGrant))
                .with_replication_policy(outbound.clone()),
        )
        .build()
        .await?;
    ...
    let pushes = outbound.pushes_of(&note_doc);
    assert_eq!(pushes.len(), 1, "one push per replicator, got {pushes:?}");
