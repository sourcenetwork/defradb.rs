...
impl<'a> SdlParser<'a> {
    ...
    pub(super) fn build_collection(
        ...
        // a policy is attached: that reaches the version and not the collection,
        // so attaching or amending one mints a new version of the same
        // collection rather than a different collection.
        ...
        let version_id = match schema::policy_commitment(policy.as_ref()) {
