...
/// Most composites indexed under one awaited CID.
...
/// Deferred composites indexed by the CIDs their verdict awaits.
///
...
impl DeferredMerges {
    ...
    /// blockstore.
    pub(crate) fn defer(&self, block: MergeBlock, awaiting: Vec<Cid>) {
        ...
        for dependency in awaiting {
            let waiters = inner.waiters.entry(dependency).or_default();
    ...
    pub(crate) fn release(&self, merged: impl IntoIterator<Item = Cid>) {
        let mut inner = self.lock();
...
impl Inner {
    fn unindex(&mut self, cid: &Cid, awaiting: &[Cid]) {
        for dependency in awaiting {
