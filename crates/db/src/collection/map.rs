...
impl CollectionMap {
    /// Record `collection` as the current version under its name.
    ///
    /// For a version this node has committed as current: it takes the name
    /// from whatever held it. A version of the same collection under another
    /// name — a rename — moves to the new one.
    pub fn put(&mut self, collection: Collection) {
        self.take_name(collection);
    }
    ...
    pub fn offer(&mut self, collection: Collection) -> Cached {
        if let Some(holder) = self.get(collection.name()) {
    ...
    fn take_name(&mut self, collection: Collection) {
        let id = collection.collection_id().to_string();
        let name = collection.name().to_string();
        if let Some(holder) = self.id_by_name.remove(&name) {
            self.evict(&holder);
        }
        self.evict(&id);
        self.id_by_name.insert(name, id.clone());
        self.id_by_version
            .insert(collection.version_id().to_string(), id.clone());
        self.by_id.insert(id, collection);
    }

    /// Drop the entry for `id` along with every index entry pointing at it.
    fn evict(&mut self, id: &str) {
        let Some(entry) = self.by_id.remove(id) else {
            return;
        };
        self.id_by_version.remove(entry.version_id());
        if self.id_by_name.get(entry.name()).map(String::as_str) == Some(id) {
