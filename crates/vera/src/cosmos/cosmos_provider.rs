...
use crate::provider::{ProviderError, ProviderPolicyInfo, SubjectRef, VeraProvider};
use crate::tuning::AcpTuning;

use super::client::{ClientError, VeraClient};
use super::tx::TxSigner;

/// Vera provider backed by Cosmos SDK (REST/LCD + CometBFT).
///
/// Wraps all network calls with a circuit breaker to prevent fail-open
/// behaviour when Vera is unreachable. Policy metadata is cached
...
impl CosmosProvider {
    ...
    /// Construct a provider with distinct Vera LCD, gRPC, and CometBFT
    /// endpoints.
    pub fn new_with_grpc(
        ...
        .map_err(|e| ProviderError::Config(format!("Vera client: {}", e)))?;
    ...
    /// Wrap a fallible Vera network call with circuit breaker logic.
    ///
        ...
        Fut: std::future::Future<Output = Result<T, ClientError>>,
    {
        if !self.circuit_breaker.allow_request() {
            tracing::warn!("Vera circuit breaker open: denying access (fail-closed)");
            return Err(ProviderError::Unavailable(
                "Vera unreachable; circuit breaker open".to_string(),
            ));
        }
...
impl VeraProvider for CosmosProvider {
    ...
    async fn create_bearer_token(&self, did: &str) -> Result<String, ProviderError> {
        ...
        tracing::warn!(
            did,
            "Vera bearer token creation failed: no signing config and no request token"
        );
