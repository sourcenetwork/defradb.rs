...
/// Transaction signer for Vera (Cosmos SDK).
///
/// Uses cosmrs for standard Cosmos SDK tx building (TxBody, AuthInfo, SignDoc,
/// signing) and hand-encoded protobuf only for Vera-specific messages
...
fn sequence_state(client: &VeraClient, address: &str) -> Arc<AsyncMutex<SequenceState>> {
...
// === Vera-specific message builders ===
// These use hand-encoded protobuf because the Vera message types
// aren't in cosmos-sdk-proto. Long-term, these could be replaced with
// prost-build compiled types from Vera's proto definitions.
