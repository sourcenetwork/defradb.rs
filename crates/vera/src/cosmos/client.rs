...
#[path = "client_naming_tests.rs"]
mod naming_tests;

/// Client for Vera ACP queries and CometBFT transaction broadcast.
pub(crate) struct VeraClient {
    /// LCD/REST base URL.
    lcd_address: String,
...
impl VeraClient {
    ...
    ///
    /// Uses Vera's gRPC API because its REST gateway cannot represent
    ...
    pub(crate) async fn verify_access(
        ...
        #[cfg(target_arch = "wasm32")]
        {
            ...
            return Err(ClientError::QueryFailed(
                "Vera access verification requires native gRPC".to_string(),
            ));
        }
...
pub(crate) enum ClientError {
    ...
    #[error("Vera unreachable (timeout): {0}")]
    Timeout(String),
...
mod tests {
    ...
    async fn grpc_access_verification_obeys_request_timeout() {
        ...
        let client = VeraClient::new(address.clone(), address.clone(), address, timeout)
