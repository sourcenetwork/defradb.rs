...
#[test]
fn signer_uses_vera_account_prefix() {
    assert!(test_signer().address().starts_with("vera1"));
}

#[test]
fn transaction_messages_use_vera_type_urls() {
    let create = build_msg_create_policy("vera1creator", "name: policy");
    assert_eq!(create.type_url, "/vera.acp.MsgCreatePolicy");
    let command =
        build_msg_bearer_policy_cmd("vera1creator", "token", "policy", serde_json::json!({}));
    assert_eq!(command.type_url, "/vera.acp.MsgBearerPolicyCmd");
}

#[tokio::test]
async fn sign_and_broadcast_holds_sequence_lock_until_inclusion() {
    let state = Arc::new(RpcState::new(ServerMode::DelayFirstTx));
    let base_url = spawn_vera_rpc(state.clone()).await;
...
async fn spawn_vera_rpc(state: Arc<RpcState>) -> String {
...
fn test_client(base_url: &str) -> VeraClient {
    VeraClient::new(
        base_url.to_string(),
        base_url.to_string(),
        base_url.to_string(),
...
fn test_signer() -> TxSigner {
    TxSigner::from_secp256k1_bytes(&[1_u8; 32], "vera-test").expect("signer should build")
}
